...
public class ParklarSayfa extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_parklar_sayfa);

        ref = FirebaseDatabase.getInstance().getReference().child("Kategoriler").child("Parklar");

        Query query = FirebaseDatabase.getInstance().getReference("Kategoriler").child("Parklar").orderByChild("yerpuan").startAt(4);

        recyclerView = findViewById(R.id.rv);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        options = new FirebaseRecyclerOptions.Builder<KategoriModeli>().setQuery(ref, KategoriModeli.class).build();
